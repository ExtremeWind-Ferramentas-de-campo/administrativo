...
function criarUsuariosExemplo() {
  ...
  Logger.log('Os dois usuários usam o SEU email, para você conseguir testar a recuperação.');
}
...
function acaoSalvarPerfil_(p) {
  ...
  const u = achado.dados;
  const permitidos = String(u.perfil).toUpperCase() === 'ADMIN' ? EDITAVEL_ADMIN : EDITAVEL_DONO;

  const mudancas = {};
  Object.keys(p.alteracoes || {}).forEach(function (chave) {
    if (permitidos.indexOf(chave) === -1) return;          // ignora silenciosamente o resto
