...
function acaoMateriaisAnexar_(p) {
  ...
  return { ok: true, driveId: arquivo.getId() };
}
...
/** { driveId } -> { ok } — manda para a lixeira, não apaga de vez. */
function acaoMateriaisRemoverAnexo_(p) {
  const matricula = validarSessao_(p.token);
  if (!matricula) return { ok: false, motivo: 'SESSAO' };

  try {
    const arquivo = DriveApp.getFileById(String(p.driveId));
    if (!estaNaPasta_(arquivo)) return { ok: false, motivo: 'NAO_ENCONTRADO' };
    arquivo.setTrashed(true);
    registrar_(matricula, 'MAT_REMOVER_ANEXO', 'OK', arquivo.getName());
  } catch (e) {
    return { ok: false, motivo: 'NAO_ENCONTRADO' };
  }
  return { ok: true };
...
function limparAnexosOrfaos() {
  ...
  for (let l = 1; l < dados.length; l++) {
    ...
    try {
      ...
      (card.attachments || []).forEach(function (a) { if (a.driveId) usados[a.driveId] = true; });
